...
export default function Home() {
  const [department, setDepartment] = useState<Department>("Visão geral");
  ...
  const chooseDepartment = (name: Department) => {
    setDepartment(name);
    if (name !== "Visão geral") toast(`${name}: módulo preparado para receber as rotinas do setor.`);
  };
    ...
    <div className="min-h-screen bg-[#F4F2EC] text-[#3C4536]">
      ...
      <main className="lg:pl-[272px]">
        ...
        <div className="container py-7 sm:py-9"><div className="mb-5 flex flex-wrap items-center gap-x-5 gap-y-2 border-l-2 border-[#D4A092] pl-3 text-[11px] text-[#7C8578]"><span className="font-extrabold uppercase tracking-[.12em] text-[#66715B]">Painel de hoje</span><span className="flex items-center gap-1.5"><span className="h-1.5 w-1.5 rounded-full bg-[#D4A092]" /> 07 pontos de atenção</span><span className="flex items-center gap-1.5"><CalendarDays className="h-3.5 w-3.5 text-[#66715B]" /> 02 vencem hoje</span><span className="hidden items-center gap-1.5 sm:flex"><ClipboardCheck className="h-3.5 w-3.5 text-[#66715B]" /> 86% das rotinas em dia</span></div>
          ...
          </> : <section className="rounded-[24px] border border-[#E1DED4] bg-[#EEEDE4] p-6 sm:p-8"><p className="eyebrow">Módulo de trabalho</p><div className="mt-3 flex flex-col justify-between gap-5 sm:flex-row sm:items-end"><div><h2 className="display-serif text-4xl text-[#485441]">Departamento {department}</h2><p className="mt-2 text-sm text-[#707A6D]">Rotinas preparadas para acompanhar cada etapa do setor.</p></div><button onClick={() => toast("A rotina será adicionada ao cliente selecionado.")} className="inline-flex items-center justify-center gap-2 rounded-xl bg-[#66715B] px-4 py-3 text-xs font-bold text-white"><Plus className="h-4 w-4" /> Nova rotina</button></div><div className="mt-7 grid gap-3 sm:grid-cols-2 lg:grid-cols-3">{taskGroups.find((g) => g.title === department)?.tasks.map((task, index) => <button key={task} onClick={() => toast(`${task}: rotina marcada para acompanhamento.`)} className="flex items-center gap-3 rounded-xl border border-[#DFDCD2] bg-[#F8F6F0] p-4 text-left hover:border-[#C5CDBB] hover:shadow-sm"><span className="flex h-7 w-7 shrink-0 items-center justify-center rounded-lg bg-[#E5EBDF] text-[#647358]"><Check className="h-4 w-4" /></span><span className="text-xs font-bold text-[#596454]">{task}</span><MoreHorizontal className="ml-auto h-4 w-4 text-[#A2A79E]" /></button>)}</div></section>}
