    ...
    <div class="modal-backdrop" id="fiscal-routine-backdrop">
      <div class="modal fiscal-routine-modal" role="dialog" aria-modal="true" aria-labelledby="fiscal-routine-modal-title">
        ...
        <div class="fiscal-routine-detail-section fiscal-routine-attachments-section">
          ...
          <div class="fiscal-routine-attachments">${attachments.length ? attachments.map((file, index) => `<button class="fiscal-routine-attachment" type="button" data-fiscal-attachment-index="${index}" aria-label="Abrir anexo ${escapeHtml(file.name)}"><span>▧</span><strong>${escapeHtml(file.name)}</strong><small>${escapeHtml(file.sizeLabel || '')}</small></button>`).join('') : '<p class="fiscal-empty-copy">Nenhum arquivo anexado a esta rotina.</p>'}</div>
        </div>
