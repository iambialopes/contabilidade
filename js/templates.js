...
function renderFiscalDashboard({ clients, selectedClient, selectedMonth, months, fiscalRows, filteredRows, filters, summary, deadlines, history, routineItems = [], routineDetails = {}, routineAlerts = [] }) {
  ...
  return `
    ...
    <section class="fiscal-hero">
      ...
      <div class="fiscal-hero-controls"><div class="fiscal-hero-competence"><span>Competência desta tela</span>${competenceSelectors(selectedMonth, getCompetenceYear(selectedMonth), months, 'fiscal')}</div><div class="fiscal-hero-client fiscal-client-picker" data-fiscal-client-picker><span>Cliente em foco desta tela</span><div class="fiscal-client-picker-field"><button class="fiscal-client-picker-trigger" type="button" id="fiscal-focus-client-trigger" aria-haspopup="listbox" aria-expanded="false" aria-controls="fiscal-focus-client-options"><span class="fiscal-client-picker-value">${selectedClient?.name || 'Selecionar cliente'}</span><span class="fiscal-client-picker-chevron" aria-hidden="true">⌄</span></button><div class="fiscal-client-picker-menu" id="fiscal-focus-client-options" role="listbox" aria-label="Selecionar cliente em foco"><input type="search" class="fiscal-client-picker-search" id="fiscal-focus-client-search" placeholder="Buscar cliente" autocomplete="off" aria-label="Buscar cliente em foco"> <div class="fiscal-client-picker-options">${[...clients].sort((a, b) => a.name.localeCompare(b.name, 'pt-BR')).map((client) => `<button class="fiscal-client-picker-option ${client.cnpj === selectedClient?.cnpj ? 'selected' : ''}" type="button" role="option" aria-selected="${client.cnpj === selectedClient?.cnpj}" data-fiscal-focus-client="${client.cnpj}" data-fiscal-client-name="${client.name.toLowerCase()} ${client.cnpj.toLowerCase()} ${client.city.toLowerCase()}"><strong>${client.name}</strong><small>${client.cnpj} · ${client.city}</small></button>`).join('')}</div></div></div><small>${selectedClient?.cnpj || ''} · ${selectedClient?.city || ''}</small></div></div>
    </section>
    ...
    <div class="modal-backdrop" id="fiscal-routine-backdrop">
      <div class="modal fiscal-routine-modal" role="dialog" aria-modal="true" aria-labelledby="fiscal-routine-modal-title">
        ...
        <div class="fiscal-routine-detail-section fiscal-routine-attachments-section">
          ...
          <div class="fiscal-routine-attachments">${attachments.length ? attachments.map((file, index) => `<div class="fiscal-routine-attachment"><button class="fiscal-routine-attachment-open" type="button" data-fiscal-attachment-index="${index}" aria-label="Abrir anexo ${escapeHtml(file.name)}"><span>▧</span><strong>${escapeHtml(file.name)}</strong><small>${escapeHtml(file.sizeLabel || '')}</small></button><button class="fiscal-routine-attachment-delete" type="button" data-fiscal-attachment-delete-index="${index}" aria-label="Excluir anexo ${escapeHtml(file.name)}" title="Excluir arquivo">×</button></div>`).join('') : '<p class="fiscal-empty-copy">Nenhum arquivo anexado a esta rotina.</p>'}</div>
        </div>
